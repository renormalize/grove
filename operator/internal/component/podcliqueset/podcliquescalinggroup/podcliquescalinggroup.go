...
func (r _resource) doCreateOrUpdate(ctx context.Context, logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet, pcsReplica int, pcsgObjectKey client.ObjectKey, pcsgConfig grovecorev1alpha1.PodCliqueScalingGroupConfig, pcsgExists bool) error {
	...
	if _, err := controllerutil.CreateOrPatch(ctx, r.client, pcsg, func() error {
		...
		r.eventRecorder.Eventf(pcs, corev1.EventTypeWarning, groveevents.ReasonPodCliqueScalingGroupCreateOrUpdateFailed, "Error creating or updating PodCliqueScalingGroup %v: %v", pcsgObjectKey, err)
		return err
	}

	r.eventRecorder.Eventf(pcs, corev1.EventTypeNormal, groveevents.ReasonPodCliqueScalingGroupCreateSuccessful, "Created PodCliqueScalingGroup %v", pcsgObjectKey)
...
func (r _resource) doDelete(ctx context.Context, logger logr.Logger, pcs *grovecorev1alpha1.PodCliqueSet, pcsgObjectKey client.ObjectKey) error {
	...
	if err := r.client.Delete(ctx, pcsg); err != nil {
		r.eventRecorder.Eventf(pcs, corev1.EventTypeWarning, groveevents.ReasonPodCliqueScalingGroupDeleteFailed, "Error deleting PodCliqueScalingGroup %v: %v", pcsgObjectKey, err)
		return groveerr.WrapError(err,
			errDeletePodCliqueScalingGroup,
			component.OperationSync,
	...
	r.eventRecorder.Eventf(pcs, corev1.EventTypeNormal, groveevents.ReasonPodCliqueScalingGroupDeleteSuccessful, "Deleted PodCliqueScalingGroup %v", pcsgObjectKey)
